...
class TransactionController extends Controller
{
    ...
    public function getDriversTransaction()
    {
        ...
        $completedDrivers = Driver::select('drivers.driver_id as driver_id', 'drivers.first_name', 'drivers.last_name', 'drivers.contact_number')
            ->leftJoin('transactions', 'drivers.driver_id', '=', 'transactions.driver_id')
            ->leftJoin('butaos', 'drivers.driver_id', '=', 'butaos.driver_id')
            ->selectRaw('COALESCE(COUNT(transactions.transaction_id), 0) as total_trips')
            ->selectRaw('COALESCE(SUM(transactions.fare_amount), 0) as total_earnings')
            ...
            ->groupBy('drivers.driver_id', 'drivers.first_name', 'drivers.last_name', 'drivers.contact_number', 'butaos.toda_commission', 'butaos.toda_paid', 'butaos.toda_balance', 'butaos.toda_payment_status')
            ->get();

        // Get drivers with "Pending" status

        $pendingDrivers = Driver::select('drivers.driver_id as driver_id', 'drivers.first_name', 'drivers.last_name', 'drivers.contact_number')
            ->leftJoin('transactions', 'drivers.driver_id', '=', 'transactions.driver_id')
            ->leftJoin('butaos', 'drivers.driver_id', '=', 'butaos.driver_id')
            ->selectRaw('COALESCE(COUNT(transactions.transaction_id), 0) as total_trips')
            ->selectRaw('COALESCE(SUM(transactions.fare_amount), 0) as total_earnings')
    ...
    public function filterPayments(Request $request)
    {
        ...
        $completedPayments = butaos::whereDate('created_at', $selectedDate)
            ->where('toda_payment_status', 'Completed')
            ->get();
        // Get all driver IDs
        $allDriverIds = Driver::pluck('driver_id')->toArray();
        // Get driver IDs with completed payments
        $completedDriverIds = $completedPayments->pluck('driver_id')->toArray();

        // Get drivers who have not completed payments

        // $pendingDriverIds = array_diff($allDriverIds, $completedDriverIds);
        // $pendingDrivers = Driver::whereIn('driver_id', $pendingDriverIds)->get();
    ...
    public function updatePayments(Request $request, $driverId)
    {
        try {
            ...
            // Redirect back with a success message
            return redirect()->route('transactions.filter')->with('success', 'Driver status updated successfully');
        } catch (\Exception $e) {
            // Log the error
            logger()->error('Error updating driver: ' . $e->getMessage());
    ...
    public function storeDriverPayment(Request $request)
    {
        ...
        return redirect()->route('transactions.filter', ['date' => $currentDate])->with('success', 'Payment recorded successfully.');
    }
}

...
{
    //
    public function getDriverInfo($rfid)
    {
        $rfid = $rfid;

        // Query the database to find the driver based on the RFID
        $driver = Driver::where('RFID', $rfid)->first();

        if (!$driver) {
            // If no driver found, return an error response
            return response()->json(['error' => 'Driver not found'], 404);
        }

        // If driver found, return the driver information
